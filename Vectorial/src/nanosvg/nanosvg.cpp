...
static void nsvg__parseContent(char* s, void(*contentCb)(void* ud,
		const char* s), void* ud) {
...
static void nsvg__parseElement(char* s, void(*startelCb)(void* ud,
		const char* el, const char** attr), void(*endelCb)(void* ud,
		const char* el), void* ud) {
...
int nsvg__parseXML(char* input, void(*startelCb)(void* ud, const char* el,
		const char** attr), void(*endelCb)(void* ud, const char* el),
...
static void nsvg__cubicBezTo(struct NSVGparser* p, float cpx1, float cpy1,
...
static struct NSVGgradientData* nsvg__findGradientData(struct NSVGparser* p,
		const char* id) {
...
static struct NSVGgradient* nsvg__createGradient(struct NSVGparser* p,
		const char* id, const float* bounds, char* paintType) {
	...
	grad = (struct NSVGgradient*) malloc(sizeof(struct NSVGgradient)
			+ sizeof(struct NSVGgradientStop) * (nstops - 1));
	if (grad == NULL)
		return NULL;
...
static void nsvg__addShape(struct NSVGparser* p) {
	...
	if (attr->hasStroke == 0) {
		...
		shape->stroke.gradient = nsvg__createGradient(p, attr->strokeGradient,
				shape->bounds, &shape->stroke.type);
...
struct NSVGNamedColor nsvg__colors[] =
		...
		{ "red", NSVG_RGB(255, 0, 0) }, { "green", NSVG_RGB( 0, 128, 0) }, {
				"blue", NSVG_RGB( 0, 0, 255) }, { "yellow",
				NSVG_RGB(255, 255, 0) }, { "cyan", NSVG_RGB( 0, 255, 255) }, {
				"magenta", NSVG_RGB(255, 0, 255) }, { "black",
				NSVG_RGB( 0, 0, 0) }, { "grey", NSVG_RGB(128, 128, 128) }, {
				"gray", NSVG_RGB(128, 128, 128) }, { "white",
...
static float nsvg__convertToPixels(struct NSVGparser* p, float val,
...
static int nsvg__parseAttr(struct NSVGparser* p, const char* name,
		const char* value) {
	float xform[6];
	struct NSVGattrib* attr = nsvg__getAttr(p);
	if (!attr)
...
static int nsvg__parseNameValue(struct NSVGparser* p, const char* start,
		const char* end) {
	const char* str;
	const char* val;
	char name[512];
...
static void nsvg__pathMoveTo(struct NSVGparser* p, float* cpx, float* cpy,
		float* args, int rel) {
	if (rel) {
		*cpx += args[0];
		*cpy += args[1];
...
static void nsvg__pathLineTo(struct NSVGparser* p, float* cpx, float* cpy,
		float* args, int rel) {
	if (rel) {
		*cpx += args[0];
		*cpy += args[1];
...
static void nsvg__pathHLineTo(struct NSVGparser* p, float* cpx, float* cpy,
		float* args, int rel) {
	if (rel)
		*cpx += args[0];
	else
...
static void nsvg__pathVLineTo(struct NSVGparser* p, float* cpx, float* cpy,
		float* args, int rel) {
	if (rel)
		*cpy += args[0];
	else
...
static void nsvg__pathCubicBezShortTo(struct NSVGparser* p, float* cpx,
		float* cpy, float* cpx2, float* cpy2, float* args, int rel) {
...
static void nsvg__pathQuadBezShortTo(struct NSVGparser* p, float* cpx,
		float* cpy, float* cpx2, float* cpy2, float* args, int rel) {
	float x1, y1, x2, y2, cx, cy;
	float cx1, cy1, cx2, cy2;
...
static void nsvg__pathArcTo(struct NSVGparser* p, float* cpx, float* cpy,
		float* args, int rel) {
		...
		while (*s) {
			...
			if (nsvg__isnum(item[0])) {
				...
				if (nargs >= rargs) {
					...
					case 'm':
					case 'M':
						nsvg__pathMoveTo(p, &cpx, &cpy, args, cmd == 'm' ? 1
								: 0);
					...
					case 'l':
					case 'L':
						nsvg__pathLineTo(p, &cpx, &cpy, args, cmd == 'l' ? 1
								: 0);
						break;
					case 'H':
					case 'h':
						nsvg__pathHLineTo(p, &cpx, &cpy, args, cmd == 'h' ? 1
								: 0);
						break;
					case 'V':
					case 'v':
						nsvg__pathVLineTo(p, &cpx, &cpy, args, cmd == 'v' ? 1
								: 0);
						break;
					case 'C':
					case 'c':
...
static void nsvg__parseRect(struct NSVGparser* p, const char** attr) {
	...
	if (w != 0.0f && h != 0.0f) {
		...
		if (rx < 0.00001f || ry < 0.0001f) {
			...
			nsvg__cubicBezTo(p, x + w - rx * (1 - NSVG_KAPPA90), y, x + w, y
					+ ry * (1 - NSVG_KAPPA90), x + w, y + ry);
			nsvg__lineTo(p, x + w, y + h - ry);
			nsvg__cubicBezTo(p, x + w, y + h - ry * (1 - NSVG_KAPPA90), x + w
					- rx * (1 - NSVG_KAPPA90), y + h, x + w - rx, y + h);
			nsvg__lineTo(p, x + rx, y + h);
			nsvg__cubicBezTo(p, x + rx * (1 - NSVG_KAPPA90), y + h, x, y + h
					- ry * (1 - NSVG_KAPPA90), x, y + h - ry);
			nsvg__lineTo(p, x, y + ry);
			nsvg__cubicBezTo(p, x, y + ry * (1 - NSVG_KAPPA90), x + rx * (1
					-NSVG_KAPPA90), y, x + rx, y);
		}
...
static void nsvg__parseCircle(struct NSVGparser* p, const char** attr) {
	...
	if (r > 0.0f) {
		...
		nsvg__cubicBezTo(p, cx + r, cy + r * NSVG_KAPPA90, cx + r
				*NSVG_KAPPA90, cy + r, cx, cy + r);
		nsvg__cubicBezTo(p, cx - r * NSVG_KAPPA90, cy + r, cx - r, cy + r
				*NSVG_KAPPA90, cx - r, cy);
		nsvg__cubicBezTo(p, cx - r, cy - r * NSVG_KAPPA90, cx - r
				*NSVG_KAPPA90, cy - r, cx, cy - r);
		nsvg__cubicBezTo(p, cx + r * NSVG_KAPPA90, cy - r, cx + r, cy - r
				*NSVG_KAPPA90, cx + r, cy);
...
static void nsvg__parseEllipse(struct NSVGparser* p, const char** attr) {
	...
	if (rx > 0.0f && ry > 0.0f) {
		...
		nsvg__cubicBezTo(p, cx + rx, cy + ry * NSVG_KAPPA90, cx + rx
				*NSVG_KAPPA90, cy + ry, cx, cy + ry);
		nsvg__cubicBezTo(p, cx - rx * NSVG_KAPPA90, cy + ry, cx - rx, cy + ry
				*NSVG_KAPPA90, cx - rx, cy);
		nsvg__cubicBezTo(p, cx - rx, cy - ry * NSVG_KAPPA90, cx - rx
				*NSVG_KAPPA90, cy - ry, cx, cy - ry);
		nsvg__cubicBezTo(p, cx + rx * NSVG_KAPPA90, cy - ry, cx + rx, cy - ry
				*NSVG_KAPPA90, cx + rx, cy);
...
static void nsvg__parsePoly(struct NSVGparser* p, const char** attr,
		int closeFlag) {
	int i;
	const char* s;
	float args[2];
...
static void nsvg__parseGradient(struct NSVGparser* p, const char** attr,
		char type) {
	int i;
	struct NSVGgradientData* grad = (struct NSVGgradientData*) malloc(
			sizeof(struct NSVGgradientData));
	if (grad == NULL)
		return;
...
static void nsvg__parseGradientStop(struct NSVGparser* p, const char** attr) {
	...
	grad->nstops++;
	grad->stops = (struct NSVGgradientStop*) realloc(grad->stops,
			sizeof(struct NSVGgradientStop) * grad->nstops);
	if (grad->stops == NULL)
		return;
...
static void nsvg__scaleGradient(struct NSVGgradient* grad, float tx, float ty,
		float sx, float sy) {
	grad->xform[0] *= sx;
	grad->xform[1] *= sx;
	grad->xform[2] *= sy;
...
static void nsvg__scaleToViewbox(struct NSVGparser* p, const char* units) {
	...
	for (shape = p->image->shapes; shape != NULL; shape = shape->next) {
		...
		if (shape->fill.type == NSVG_PAINT_LINEAR_GRADIENT || shape->fill.type
